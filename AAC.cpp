...
AAC_CompileResult AAC::CompileFromAbstractSyntaxTrees(std::vector<AA_AST*> trees) {
	...
	for (size_t i = 0; i < trees.size(); i++) {
		...
		if (trees[i]->GetRoot()->type == AA_AST_NODE_TYPE::classdecl) {
			...
			// Generated bytecode
			compileResults.push_back(CompileProcedureFromAST(trees[i], senv));
...
AAC::CompiledProcedure AAC::CompileProcedureFromAST(AA_AST* pAbstractTree, AAStaticEnvironment staticData) {
...
AAC_Out AAC::CompileFromProcedures(std::vector<CompiledProcedure> procedures, AAStaticEnvironment staticCompileData, int entryPoint) {
